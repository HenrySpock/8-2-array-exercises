// 1.
// Write a function called doubleValues which accepts an array and returns a new array 
// with all the values in the array passed to the function doubled

// doubleValues([1,2,3]) // [2,4,6]
// doubleValues([5,1,2,3,10]) // [10,2,4,6,20]

let numVals = [1, 2, 3, 4, 10, -15, 62]; 
// let doubleArray = [];

// function doubleValues(val) {  
    // doubleArray.push(val*2);
    // doubleArray.pop; (this removes the NaN of undefined)
    // return doubleArray;
// }

// numVals.forEach(doubleValues);

// 2.
// Write a function called onlyEvenValues which accepts an array and returns a new array with 
// only the even values in the array passed to the function

// onlyEvenValues([1,2,3]) // [2]
// onlyEvenValues([5,1,2,3,10]) // [2,10]

// let evenArray = [];
// function onlyEvenValues(val) {  
//     if (val % 2 === 0) {evenArray.push(val)};
//     evenArray.pop; // (this removes the NaN of undefined)
//     console.log(evenArray);
//     return evenArray;
//     }

// numVals.forEach(onlyEvenValues); //uses numVals from exercise 1

// 3. showFirstAndLast
// Write a function called showFirstAndLast which accepts an array of strings and returns a new array 
// with only the first and last character of each string.

// showFirstAndLast(['colt','matt', 'tim', 'test']) // ["ct", "mt", "tm", "tt"]
// showFirstAndLast(['hi', 'goodbye', 'smile']) // ['hi', 'ge', 'se']

// let flExamples = ['Michael', 'Henry', 'Tillman']
// let showFL = [];

// function showFirstAndLast(val) {
//     showFL.push(val.charAt(0) + (val.charAt(val.length -1)));
//     return showFL;
// }

// flExamples.forEach(showFirstAndLast); 

// 4. addKeyAndValue 
// Write a function called addKeyAndValue which accepts an array of objects, a key, 
// and a value and returns the array passed to the function with the new key and value 
// added for each object

// const names = [
//     {name: 'Elie'},
//     {name: 'Tim'},
//     {name: 'Matt'},
//     {name: 'Colt'}
//     ]

// const addKeyAndValue = (arr, key, value) => {
//     arr.forEach(object => {
//     object[key] = value
//     }) 
//     return arr
//     }
    
// addKeyAndValue(names, 'title', 'instructor')

// 5. 
// vowelCount
// Write a function called vowelCount which accepts a string and returns an object 
// with the keys as the vowel and the values as the number of times the vowel appears 
// in the string. This function should be case insensitive so a lowercase letter and 
// uppercase letter should count.

// vowelCount('Elie') // {e:2,i:1};
// vowelCount('Tim') // {i:1};
// vowelCount('Matt') // {a:1})
// vowelCount('hmmm') // {};
// vowelCount('I Am awesome and so are you') // {i: 1, a: 4, e: 3, o: 3, u: 1}

 
// let vowelNums = {};

// function vowelCount(str) {
// 	let vowels = "aeiou";
// 	let strLow = str.toLowerCase();
// 	for (let i = 0; i < vowels.length; i++) {
//     	let key = vowels[i];
//     	if (strLow.includes(vowels[i])) {vowelNums[key] = (strLow.split(vowels[i]).length-1)}
	
// }
// return vowelNums;
// }



// map

// 1.
// doubleValuesWithMap
// Write a function called doubleValuesWithMap which accepts an array and returns 
// a new array with all the values in the array passed to the function doubled
 
// let numArray = [1, 5, 8, 17, 26];

// function doubleValuesWithMap(arr) {
//     return arr.map(function(val) {
//       return val * 2;
//     });
//   }

// let arrDoubles = doubleValuesWithMap(numArray);
  
// 2.
// valTimesIndex
// Write a function called valTimesIndex which accepts an array and returns a new array 
// with each value multiplied by the index it is currently at in the array.

// valTimesIndex([1,2,3]) // [0,2,6]
// valTimesIndex([1,-2,-3]) // [0,-2,-6]

let timesIndex = [1, -5, 8, -17, 26];

function valTimesIndex(arr) {
    return arr.map(function(val, idx) {
      return val * idx;
    });
  }

let indexMultiplier = valTimesIndex(timesIndex);

// 3.
// extractKey
// Write a function called extractKey which accepts an array of objects and 
// some key and returns a new array with the value of that key in each object.

let someRandomNames =   [
    {name: 'Elie'},
    {name: 'Tim'},
    {name: 'Matt'},
    {name: 'Colt'}
  ]; 
  
//   function extractKey(arr) {
//         let justNames = [];
//         for(let i = 0; i < arr.length; i++) {
//         justNames.push(Object.values(someRandomNames[i]))};
//         let noReallyJustNames = justNames.flat();
//         return noReallyJustNames;
//   }

//   function extractKey(arr, key) {
//     return arr.map(function(val) { 
//       return val[key];
//     });
//   }
 
// let justTheRandom = extractKey(someRandomNames, 'name');
 
// 4. 
// extractFullName
// Write a function called extractFullName which accepts an array of objects and 
// returns a new array with the value of the key with a name of “first” and the 
// value of a key with the name of “last” in each object, concatenated together 
// with a space.


let firstAndLast = ([
  {first: 'Elie', last:"Schoppik"},
  {first: 'Tim', last:"Garcia"},
  {first: 'Matt', last:"Lane"},
  {first: 'Colt', last:"Steele"}
])

  // ['Elie Schoppik', 'Tim Garcia', 'Matt Lane', 'Colt Steele']


function extractFullName(arr) {
        return arr.map(function(val) { 
          return val.first + " " + val.last;
        });
      }
     
let fullNames = extractFullName(firstAndLast);

// filter 

// 1.
// filterByValue
// Write a function called filterByValue which accepts an array of objects and a key 
// and returns a new array with all the objects that contain that key.

// petLovers =
// [
//   {first: 'Elie', last:"Schoppik"},
//   {first: 'Tim', last:"Garcia", isCatOwner: true},
//   {first: 'Matt', last:"Lane"},
//   {first: 'Colt', last:"Steele", isCatOwner: true}
// ];

//   [
//     {first: 'Tim', last:"Garcia", isCatOwner: true},
//     {first: 'Colt', last:"Steele", isCatOwner: true}
//   ] 

// function filterByValue(arr, key) {
//     return arr.filter(function(val) {
//       return val[key] !== undefined;
//     });
//   }
 
// let catOwners = filterByValue(petLovers, 'isCatOwner');

// 2.
// find
// Write a function called find which accepts an array and a value and returns 
// the first element in the array that has the same value as the second 
// parameter or undefined if the value is not found in the array.

// find([1,2,3,4,5], 3) // 3
// find([1,2,3,4,5], 10) // undefined

// function find(arr, firstEl) {
//     return arr.filter(function(val) {
//       return val === firstEl;
//     })[0];
//   }
  
// 3.
// findInObj
// Write a function called findInObj which accepts an array of objects, a key, and some 
// value to search for and returns the first found value in the array.

// let findTheCatters =
//   [
//     {first: 'Elie', last:"Schoppik"},
//     {first: 'Tim', last:"Garcia", isCatOwner: true},
//     {first: 'att', last:"Lane"},
//     {first: 'Colt', last:"Steele", isCatOwner: true}
//   ];
 

// // {first: 'Tim', last:"Garcia", isCatOwner: true}

// function findInObj(arr, key, firstEl) {
//     return arr.filter(function(val) {
//       return val[key] === firstEl;
//     })[0];
//   }

// let firstKnownCatter = findInObj(findTheCatters, 'isCatOwner', true);

// 4.
// removeVowels
// Write a function called removeVowels which accepts a string and returns a 
// new string with all of the vowels (both uppercased and lowercased) removed. 
// Every character in the new string should be lowercased.

// removeVowels('Elie') // ('l')
// removeVowels('TIM') // ('tm')
// removeVowels('ZZZZZZ') // ('zzzzzz')

// function removeVowels(str) {
//     let vowels = "aeiou";
//     return str.toLowerCase().split("")
//     .filter(function(val) {
//       return vowels.indexOf (val) === -1;
//     }).join("");
//   }
 
// 5.
// doubleOddNumbers
// Write a function called doubleOddNumbers which accepts an array and 
// returns a new array with all of the odd numbers doubled (HINT - 
//     you can use map and filter to double and then filter the odd numbers).

// let numberArray = [1,2,3,4,5, 21, 42, 53, 655]

// function doubleOddNumbers(arr) {
//     return arr.filter(function(val) {
//         return val % 2 !== 0;
//       })
//       .map(function(val) {
//         return val * 2;
//       });
//     }